...
# 選單函數
show_menu() {
    grep -E "COLDADDRESS=0x[0-9a-fA-F]{40}" "$CONFIG_FILE/config.txt" | awk -F '=' '{print $1 "\n" $2}'
    grep -E "HOTADDRESS=0x[0-9a-fA-F]{40}" "$CONFIG_FILE/config.txt" | awk -F '=' '{print $1 "\n" $2}'
...
DLP_MOKSHA_CONTRACT=0x50547C657F60dAd7887d527173B86871b11921eF

# Optional: Your own DLP token contract address once deployed to the network, useful for local testing
DLP_TOKEN_MOKSHA_CONTRACT=0xBC462f38ACF8CFA509e4dD448F07d33DdE41a03E
...
EOL
    ...
    echo "設置驗證器..."
    cd $HOME/vana-dlp-chatgpt
    ./vanacli dlp register_validator --stake_amount 500 || { echo "註冊驗證器失敗"; exit 1; }

    grep -E "HOTADDRESS=0x[0-9a-fA-F]{40}" "$LOG_DIR/config.txt" | awk -F '=' '{print $1 "\n" $2}'
}
